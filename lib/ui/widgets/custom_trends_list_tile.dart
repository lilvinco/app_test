...
enum ListTileStyle {
  ...
  /// Use a title font that's appropriate for a [ListTile] that
  /// appears in a [Drawer].
  drawer,
}
...
class ListTileTheme extends InheritedWidget {
  ...
  /// If specified, the color used for icons and text when a [ListTile]
  /// is selected.
  final Color? selectedColor;

  /// If specified, the icon color used for enabled [ListTile]s
  /// that are not selected.
  final Color? iconColor;

  /// If specified, the text color used for enabled [ListTile]s
  /// that are not selected.
  ...
  static ListTileTheme of(BuildContext context) {
    final ListTileTheme? result =
        context.dependOnInheritedWidgetOfExactType(aspect: ListTileTheme);
    return result ?? ListTileTheme(child: context.widget);
  }
...
///
/// A list tile contains one to three lines of text optionally flanked by
/// icons or
...
///   subtitle: _act != 2 ? const
/// Text('The airplane is only in Act II.') : null,
///   enabled: _act == 2,
...
///         viewCount: 999000,
///         thumbnail: Container(
///           decoration: const BoxDecoration(color: IGrooveTheme.colors.blue),
///         ),
...
///         thumbnail: Container(
///           decoration:
///  const BoxDecoration(color: IGrooveTheme.colors.yellow),
///         ),
...
///       CustomListItemTwo(
///         thumbnail: Container(
///           decoration: const BoxDecoration(color: IGrooveTheme.colors.pink),
///         ),
...
class TrendsCustomListTile extends StatelessWidget {
  /// Creates a list tile.
  ///
  /// If [isThreeLine] is true, then [subtitle] must not be null.
  ///
  /// Requires one of its ancestors to be a [Material] widget.
  const TrendsCustomListTile({
    Key? key,
    this.leading,
    this.title,
    this.subtitle,
  ...
  /// If this tile is also [enabled] then icons and text are rendered
  ///  with the same color.
  ///
  ...
  static Iterable<Widget> divideTiles(
    ...
    final Decoration decoration = const BoxDecoration();
  ...
  Color? _textColor(
    ...
    if (selected) {
      switch (theme.brightness) {
        case Brightness.light:
          return IGrooveTheme.colors.primary;
        case Brightness.dark:
  ...
  void _updateRenderObject(RenderObject? child, _ListTileSlot? slot) {
    switch (slot) {
      ...
      case _ListTileSlot.subtitle:
        renderObject.subtitle = child as RenderBox?;
        break;
