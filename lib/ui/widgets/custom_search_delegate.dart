...
import 'package:igroove_fan_box_one/helpers/svg_icons_icons.dart';
...
/// [.close] call. If the search page is closed with the system
/// back button, it returns null.
///
/// A given [] can only be associated with one active [showSearch]
/// call. Call [.close] before re-using the same delegate instance
...
/// The animation provided by [.transitionAnimation] can be used
...
/// See also:
///
///  * [] to define the content of the search page.
...
/// query text field can be customized via [CustomSearchDelegate.leading] and
/// [CustomSearchDelegate.actions].
///
/// The body below the [AppBar] can either show suggested queries (returned by
/// [CustomSearchDelegate.buildSuggestions]) or - once the user submits a
/// search  - the
/// results of the search as returned by [CustomSearchDelegate.buildResults].
///
/// [CustomSearchDelegate.query] always contains the current query entered
/// by the user
/// and should be used to build the suggestions and results.
///
/// The results can be brought on screen by calling
///  [CustomSearchDelegate.showResults]
/// and you can go back to showing the suggestions by calling
/// [CustomSearchDelegate.showSuggestions].
///
/// Once the user has selected a search result, [CustomSearchDelegate.close]
/// should be
...
///
/// A given [CustomSearchDelegate] can only be associated with one active
///  [showSearch]
/// call. Call [CustomSearchDelegate.close] before re-using the same delegate
///  instance
/// for another [showSearch] call.
abstract class CustomSearchDelegate<T> {
  ...
  ///  * [AppBar.systemOverlayStyle], which is set to
  // ignore: deprecated_member_use
  ///  [ThemeData.primaryColorBrightness].
  ThemeData appBarTheme(BuildContext context) {
    final ThemeData theme = Theme.of(context);
    return theme.copyWith(
      primaryColor: IGrooveTheme.colors.white,
      primaryIconTheme:
          theme.primaryIconTheme.copyWith(color: IGrooveTheme.colors.grey),
...
enum _SearchBody {
  ...
  ///
  /// The suggested queries are generated
  ///  by [CustomSearchDelegate.buildSuggestions].
  suggestions,
...
class _SearchPageState<T> extends State<_SearchPage<T>> {
  ...
  Widget build(BuildContext context) {
    ...
    return Semantics(
      ...
      label: routeName,
      child: Scaffold(
        appBar: IGrooveAppBarWidget.onlyWidget(
          title: Row(
            children: <Widget>[
              Expanded(
                child: Container(
                        ...
                        child: Container(
                          child: TextField(
                            ...
                            style: theme.primaryTextTheme.headline2!.copyWith(
                                fontSize: 15, color: IGrooveTheme.colors.black),
