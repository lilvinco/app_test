...
class _PopupMenuRouteLayout extends SingleChildLayoutDelegate {
  ...
  Offset getPositionForChild(Size size, Size childSize) {
    ...
    if (position!.left > position!.right) {
      // Menu button is closer to the right edge, so grow to the left,
      // aligned to the right edge.
      x = size.width - position!.right - childSize.width;
    } else if (position!.left < position!.right) {
      // Menu button is closer to the left edge, so grow to the right,
      //aligned to the left edge.
      x = position!.left;
    } else {
      // Menu button is equidistant from both edges, so grow in reading
      // direction.
...
///  * [CustomPopupMenuButton], which provides an [IconButton] that shows a
///  * menu by
...
/// Signature for the callback invoked when a [CustomPopupMenuButton]
/// is dismissed
...
/// Signature used by [CustomPopupMenuButton] to lazily construct the
/// items shown when
...
/// dismissed
/// because an item was selected. The value passed to [onSelected] is the
/// value of
/// the selected menu item.
///
/// One of [child] or [icon] may be provided, but not both. If [icon] is
/// provided,
...
///  * [PopupMenuDivider], a popup menu entry that is just a horizontal line.
///  * [CheckedPopupMenuItem], a popup menu item with a checkmark.
///  * [showMenu], a method to dynamically show a popup menu at a given
///  * location.
class CustomPopupMenuButton<T> extends StatefulWidget {
  ...
  /// Matches IconButton's 8 dps padding by default. In some cases,
  /// notably where
  /// this button appears as the trailing element of a list item, it's
  ///  useful to be able
